...
- **File tools** (`Read`, `Glob`, `Grep`) with access to `__USER_HOME__/projects/se-core/` AND `__PROJECTS_ROOT__/<project>/`. Use these for all manifest / lockfile reads.
...
Write to `__USER_HOME__/projects/se-core/audits/YYYY-MM-DD-dep-currency.md`:
